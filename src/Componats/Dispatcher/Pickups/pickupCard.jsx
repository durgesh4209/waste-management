...
const PickupCardDispatcher = ({ order }) => {
    ...
    const handlePickup = () => {
        api.get(`/ecobin/dispatcher/pickups/${order.id}`)
            .then(response => {
            ...
            .catch(error => {
                setAlertType('error');
                setShowAlert(true);
                setMsg('Error unable to pick the order. Please try again.');
    ...
    return (
        <div className="container mt-5">
            <div className="card shadow-sm p-4 mb-4">
                ...
                <div className="d-flex mb-4 border-bottom pb-3 align-items-center">
                    ...
                    <div className="flex-grow-1 m-l-200">
                        <h6 className="fw-bold">
                            ...
                            {order1.completed
                                ? `Picked up on ${new Date(order1.pickupDate).toLocaleDateString("en-US", {
                                    year: "numeric",
                                    month: "long",
                                    day: "numeric",
                                })}`
                                : `Pick up scheduled on ${new Date(order1.pickupDate).toLocaleDateString("en-US", {
                                    year: "numeric",
                                    month: "long",
                                    day: "numeric",
                                })}`}
                ...
                <span>Address :</span>
                        <p className="text-muted d-inline-block">{order.pickupAddress.street} {order.pickupAddress.city} {order.pickupAddress.postalCode}</p>
                ...
                <div className="d-flex justify-content-end align-items-center mt-3 w-100">
                    ...
                    {order1.completed === false ? (  // Show button when isCompleted is false
                        <button className="btn btn-danger" style={{ width: 100 }} onClick={handlePickup}>
                            Pickup
                        </button>
                    ) : (
...
export default PickupCardDispatcher;

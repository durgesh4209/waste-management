...
const Card = ({ order }) => {
    ...
    const handleShipOrder = () => {
        api.get(`/ecobin/dispatcher/ship/id/${order.id}`)
            .then(response => {
                setAlertType('success');
                setShowAlert(true);
                setMsg('Order Shipped successfully!');
    ...
    const handleDeliveredOrder = () => {
        debugger
        api.get(`/ecobin/waste/order/status/Delivered/id/${order.id}`)
            .then(response => {
                setAlertType('success');
                setShowAlert(true);
                setMsg('Order Shipped successfully!');
            ...
            <div className="card shadow-sm p-4 mb-4">
                ...
                {order.wastes.map((waste, index) => (
                    <div key={index}>
                        ...
                        <span>Address:-</span>
                        <p className="text-muted d-inline-block">{order.shippingAddress.address} {order.shippingAddress.city} {order.shippingAddress.zipCode}</p>

                    </div>
                ))}

                <div className="d-flex justify-content-end align-items-center mt-3 w-100">
                    <button className="btn btn-danger " hidden >Ship</button>
                    {(() => {
                        if (order.orderStatus === "Delivered") {
                            return <button className="btn btn-primary" hidden>Cancelled</button>;
                        } else if (order.orderStatus === "Confirmed") {
                            return (
                                <button className="btn btn-danger" style={{ width: 100 }} onClick={handleShipOrder}>
                                    Ship
                                </button>
                            );
                        } else if (order.orderStatus === "Shipped") {
                            return (
                                <button className="btn btn-danger" style={{ width: 100 }} onClick={handleDeliveredOrder}>
                                    Delivered
                                </button>
                            );
                        }
